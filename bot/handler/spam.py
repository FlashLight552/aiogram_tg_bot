...
async def cancel_spam(message : types.message, state: FSMContext):
    ...
    await message.answer('Отмена', reply_markup=main_btn, disable_notification=True)
...
def handlers_spam(dp: Dispatcher):
    ...
    dp.register_message_handler(cancel_spam, Text(equals = 'Отмена', ignore_case = True), state="*" )
    # Подтверждение
